...
var events = [];
var moreEventsDeleted = false;

var DATE_SEPARATOR = "/";
// default - get 10 events
var itemsAtPage = 10; // how many items on the page at the same time, how many to get at each "more events" click
var eventCount = 0; // event count on the page
var eventArea = $("#events"); // event area
var modalArea = $("#modals"); // modal area
var moreEventsField =
  '<div class="well well-md text-center"> <h3>More Events &nbsp;<i class="fa fa-arrow-circle-down"></i></h3> <!-- a fucking credit to @onrcskn  --> </div>';
  ...
  for (var i in events) {
    if (events.hasOwnProperty(i)) {
      ...
      if (!events[i].place.hasOwnProperty("location") || !events[i].place.location.hasOwnProperty("latitude") || !
        ...
        events[i].place.location = {
          latitude: "",
          longitude: ""
        };
      }
      if (events[i].ticket_uri === undefined || events[i].ticket_uri === null) events[i].ticket_uri =
        "https://www.facebook.com/events/" + events[i].id + "/";
      ...
      var eventGridData = "<!-- event -->" +
        ...
        '<h5 class="title title-modern" style="margin-top:0"><i style="opacity:0.8">by ' + events[i].owner.capitalize() +
        '</i></h5>' +
      ...
      var modalData = '<!-- modal -->' +
        '<div class="modal modal-fullscreen fade" id="detail' + eventCount +
        '" tabindex="-1" role="dialog" aria-labelledby="label' + eventCount + '" aria-hidden="true">' +
        ...
        '<small id="dateModalText"> ' + startDate + " " + startHour + (endDate !== null && endDate !== undefined ? (
          ' - ' + endDate + " " + endHour) : "") + '</small>' +
        '</div>' +
        ...
        '<h6>Description</h6>' +
        '<hr style="margin-bottom: 10px"/><span style="white-space: pre-line;"><small>' + events[i].description +
        '</small></span></div>' +
...
$(document).ready(function() {
  ...
  for (var i = clubs.length - 1; i > -1; i--) {
    if (clubs.hasOwnProperty(i))
      clubDivider.after('<li class="single"><label class="checkbox" for="checkbox' + i1 +
        '" style="margin-left: 5%"><span class="icons"><span class="first-icon fa fa-square fa-base"></span><span class="second-icon fa fa-check-square fa-base"></span></span><input class="club" type="checkbox" value="' +
        clubs[i][0] + '" id="checkbox' + (i1++) + '" data-toggle="checkbox">' + clubs[i][1] + '</label></li>');
  }

  var categories = process("getCategories", null);
  for (i = categories.length - 1; i > -1; i--) {
    if (categories.hasOwnProperty(i))
      catDivider.after('<li class="single"><label class="checkbox" for="checkbox' + i1 +
        '" style="margin-left: 5%"><span class="icons"><span class="first-icon fa fa-square fa-base"></span><span class="second-icon fa fa-check-square fa-base"></span></span><input class="category" type="checkbox" value="' +
        categories[i] + '" id="checkbox' + (i1++) + '" data-toggle="checkbox">' + categories[i].capitalize() +
        '</label></li>');
  }
